...
echo "=== syntax check ==="
uv run python -m compileall -q config.py split_dataset.py ga.py train_final.py bootstrap.py pretrain.py analyze_test_errors.py check_data.py create_summary_table.py plot_fitness_evolution.py cnn

echo "=== import smoke test (config + scripts + cnn package) ==="
uv run python -c "import config, split_dataset, ga, train_final, bootstrap, pretrain, analyze_test_errors, plot_fitness_evolution, check_data, cnn.engine, cnn.data_setup, cnn.model; print('imports OK')"
