...
from cnn.util.datautils import HypercubeDataset
...
def check_dataset(csv_path, band_indices, output_dir, num_images=8):
    ...
    if len(dataset) < num_images:
        print(f"⚠️ Warning: Dataset has fewer than {num_images} samples. Displaying all {len(dataset)} samples.")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--csv', type=str, default='train_dataset.csv', help='Path to the dataset CSV file to check.')
    parser.add_argument('--bands', type=str, default='50,80,120', help='Comma-separated band indices.')
    parser.add_argument('--output_dir', type=str, default='sanity_check_output', help='Directory to save the output images.')
